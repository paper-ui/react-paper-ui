...
import { ce, now } from '../../core/utils';
import ElementRect from '../../core/ElementRect';
 ...
 * @class PaperWave
 */
export default class RippleWave {
    ...
    constructor({ $, recenters = false, center = false, initialOpacity = 0.25, opacityDecayVelocity = 0.8 }) {
        if (!$) {
            throw new ReferenceError('$ is null');
        }
         ...
         * @type {String}
         */
        this.color = window.getComputedStyle($).color;
    ...
    get touchDownElapsed() {
        ...
        elapsed = now() - this.touchDownStarted;
     ...
     */
    get touchUpElapsed() {
        return this.touchUpStarted ? now() - this.touchUpStarted : 0;
    }
    ...
    get radius() {
        const radius = Math.min(
                ...
                RippleWave.MAX_RADIUS
                ) * 1.1 + 5,
            elapsed = 1.1 - 0.2 * (radius / RippleWave.MAX_RADIUS),
    ...
    get isWaveFullyOpaque() {
        return this.opacity < 0.01 && this.radius >= Math.min(this.maxRadius, RippleWave.MAX_RADIUS);
    }
    ...
    get isMaxRadiusReached() {
        return this.opacity >= this.initialOpacity && this.radius >= Math.min(this.maxRadius, RippleWave.MAX_RADIUS);
    }
